...
/**
 * a customized of version of the TooltipManager class of the swing which is
 * used for logging when users view tooltips in a UI
 * 
 * @author arin ghazarin
 * 
 */
public class MirrorToolTipManager extends MouseAdapter implements
		MouseMotionListener {
	...
	final static MirrorToolTipManager sharedInstance = new MirrorToolTipManager();
	...
	void showTipWindow() {
		if (insideComponent == null || !insideComponent.isShowing())
			return;
		for (Container p = insideComponent.getParent(); p != null; p = p
				.getParent()) {
		...
		if (enabled) {
			...
			// tipWindow.show();
			Logger.getInstance().logThisToolTipShowedEvent(toolTipText);
	...
	void hideTipWindow() {
		if (tipWindow != null) {
			...
			Logger.getInstance().logThisToolTipHidededEvent(toolTipText);
			tipWindow = null;
			tipShowing = false;
	 ...
	 */
	public static MirrorToolTipManager sharedInstance() {
	...
	private void show(JComponent source) {
		if (tipWindow != null) { // showing we unshow
			...
			if (insideComponent != null) {
				...
				preferredLocation = new Point(10,
						insideComponent.getHeight() + 10); // manual set
				showTipWindow();
	...
	private static class Actions extends UIAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (key == SHOW) {
				MirrorToolTipManager.sharedInstance().show(source);
			} else if (key == HIDE) {
				MirrorToolTipManager.sharedInstance().hide(source);
			}
		}
		...
		public boolean isEnabled(Object sender) {
			...
			return MirrorToolTipManager.sharedInstance().tipShowing;
		}
	}
}
